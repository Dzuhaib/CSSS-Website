...
function DropdownMenu({
  ...
  return (
    ...
    >
      <button
        className={`px-4 py-2 text-sm font-medium transition-all duration-300 rounded-full flex items-center gap-1 ${isActive
            ? isScrolled
      ...
      <div
        className={`absolute top-full left-0 mt-2 w-56 bg-white/95 backdrop-blur-xl rounded-xl shadow-xl border border-border/50 overflow-hidden transition-all duration-200 ${isOpen ? "opacity-100 visible translate-y-0" : "opacity-0 invisible -translate-y-2"
          }`}
      >
        {item.children?.map((child) => (
          <Link
            key={child.href}
            href={child.href}
            className={`block px-4 py-3 text-sm transition-colors ${location === child.href
          ...
          {NAV_LINKS.map((link) => (
            link.children ? (
              ...
              <Link
                ...
                className={`px-4 py-2 text-sm font-medium transition-all duration-300 rounded-full relative group ${location === link.href
                    ? isScrolled
                      ? "text-primary bg-primary/5"
                      : "text-white bg-white/15"
                    : isScrolled
              ...
              >
                {link.label}
                {location === link.href && (
                  <span className={`absolute bottom-0 left-1/2 -translate-x-1/2 w-1 h-1 rounded-full ${isScrolled ? "bg-accent" : "bg-white"
                    }`} />
                )}
              </Link>
            )
                ...
                link.children ? (
                  <div key={link.label}>
                    <button
                      ...
                      className={`w-full text-lg font-display py-3 px-4 rounded-xl transition-all duration-300 flex items-center justify-between ${expandedMobile === link.label
                          ? "text-primary bg-primary/5 font-semibold"
                          : "text-foreground hover:text-primary hover:bg-muted/50"
                        }`}
                    >
                      {link.label}
                      <ChevronDown className={`w-5 h-5 transition-transform duration-200 ${expandedMobile === link.label ? "rotate-180" : ""
                        }`} />
                    </button>
                    <div className={`overflow-hidden transition-all duration-300 ${expandedMobile === link.label ? "max-h-96" : "max-h-0"
                      }`}>
                      {link.children.map((child) => (
                        <Link
                          ...
                          className={`block py-2.5 pl-8 pr-4 text-base transition-colors ${location === child.href
                  ...
                  <Link
                    ...
                    href={link.href}
                    onClick={() => setIsOpen(false)}
                    className={`text-lg font-display py-3 px-4 rounded-xl transition-all duration-300 ${location === link.href
                        ? "text-primary bg-primary/5 font-semibold"
                        : "text-foreground hover:text-primary hover:bg-muted/50"
                      }`}
                  >
